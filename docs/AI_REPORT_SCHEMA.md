...
- 防止 AI 创建新的 Finding、Cause ID、Action ID 或未经验证的业务事实数字。
- 保证 Provider 失败或输出非法时，确定性诊断仍然独立可用。

本文定义的是产品数据契约，不绑定具体 Provider、模型或 SDK 实现。后续代码通过 `AIProvider` 接口使用该契约，第一版真实演示使用 DeepSeek Provider，但 Schema 不因此增加厂商专属字段。
...
AI不得生成 Payload 或 Catalog 之外的未经验证业务事实数字。AI允许引用 Evidence 中已经提供的业务数字，也允许生成建议性阈值，但必须明确标注为“建议”“可作为观察条件”或同等含义，不得将建议阈值表达为当前业务事实。

数字边界：

- 当前销售额、变化率、ROAS、库存天数等经营事实，必须来自 Payload 中的确定性 KPI、Finding 和 Evidence。
- 负责人、建议周期、观察指标等行动信息，必须来自对应 Action Catalog。
- AI生成的阈值只用于建议如何观察行动效果，不代表系统已经计算或验证该阈值。
- 页面仍根据 `finding_id` 和 `action_id` 渲染确定性 Evidence 与 Catalog 信息，并将AI建议和当前业务事实明确区分。
...
- 允许为空：否。
- 用途：综合当前筛选范围内的主要经营状态。
- 约束：可以引用 Evidence 中已有数字；建议性阈值必须明确为建议，不能伪装成当前经营事实，也不能创建新的经营问题。
...
| `reason` | 字符串 | 是 | 解释为什么建议按该顺序执行；如包含AI生成阈值，必须明确标注为建议性判断条件 |
...
AI可以在 `reason` 中引用 Evidence 或 Catalog 已有数字，也可以提出建议性阈值；建议性阈值不属于确定性事实，Validator 第一版不审核其自然语言真实性，页面和文案必须保留“建议”语义。
...
5. 行动计划按照 `action_sequence` 展示 `reason`；观察指标、负责人和建议周期由系统根据 `action_id` 从 Action Catalog 渲染；AI生成阈值必须按建议性判断条件展示。
...
- AIProvider：定义与具体模型服务解耦的调用接口，只传递白名单输入并返回待校验 JSON；第一版真实演示使用的 DeepSeek Provider 必须遵守同一接口和 Schema。
- 页面：只渲染已通过校验的 AIReport，并从确定性系统补充业务事实数字和 Action Catalog 信息；AI建议性阈值不得与确定性事实混淆。
