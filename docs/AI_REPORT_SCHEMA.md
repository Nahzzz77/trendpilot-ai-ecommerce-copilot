...
本文定义的是产品数据契约，不包含具体 Provider、模型或 SDK 实现。
...
| `action_priority` | 字符串枚举 | 是 | 仅允许 `high`、`medium`、`low`，表示行动执行优先级 |
| `reason` | 字符串 | 是 | 解释为什么建议按该优先级执行 |

数组展示顺序为 `high`、`medium`、`low`。`action_priority` 不得修改或替代 Finding 的规则 Priority。
...
5. 行动计划展示 `action_priority` 和 `reason`；观察指标、负责人和建议周期由系统根据 `action_id` 从 Action Catalog 渲染。
