...
→ 生成经过结构与引用校验的 AI 经营分析报告
```

所有经营数字和问题判断都有明确来源。当前版本已经完成 AI Report MVP：页面通过统一 Service 调用 Provider，模型只接收白名单 KPI、诊断结果和受控候选目录，输出通过 Validator 后才会展示。项目已实现 OpenAI Provider，但尚未进行真实 OpenAI 效果评估；无 API Key 或调用失败时，确定性诊断仍然可用。

## 产品架构

TrendPilot 使用“确定性分析 + AI 增强”的混合架构：
...
- **AI Report Service** 统一编排白名单 Payload、Provider 调用和报告校验，页面不直接调用模型 SDK。
- **OpenAI Provider** 是第一版真实 Provider 实现；**Fake Provider** 保留用于无网络测试和稳定演示验证。
- **LLM** 只在结构化诊断结果之上负责经营总结、问题解释、原因假设和行动排序，不重新计算指标，也不直接读取原始 CSV。
- **Report Validator** 校验 JSON 结构以及 Finding、Cause、Action 引用；未通过校验的报告不会展示。
...
- 在确定性诊断结果之后提供独立的“AI增强分析”区域。
- 展示经营摘要、重点问题解释、问题关联、原因假设、验证方式、行动顺序和数据限制。
- AI 只接收当前筛选范围内的白名单 KPI、Finding、Evidence 及对应 Cause/Action 候选。
- 支持环境变量或 Streamlit secrets 配置 OpenAI 服务，不在代码中保存 API Key。
- 覆盖无 Key、Provider 失败、Validator 失败和无 Finding 等降级状态，不影响确定性诊断。
- 筛选范围变化时自动使旧报告失效，避免跨分析范围复用结果。
- 已建立 5 个黄金业务场景的自动约束检查与人工评估记录模板；真实 OpenAI 报告质量仍待评估。

### 测试

- 当前共有 **136 个自动测试**。
- 覆盖数据加载、字段校验、指标计算、图表、Dashboard、DiagnosisContext、规则边界、优先级、目录引用、诊断页面、AI Report 数据层、Provider、Service、页面集成和黄金场景约束。
...
| Phase 3：AI Report MVP | 已完成 | 白名单 Payload、Prompt、Provider、Validator、报告页面与评估基线 |
...
src/
  ...
  ai_report_models.py         # AI Report 数据结构与生成状态
  ai_report_payload.py        # 白名单模型输入构建
  ai_report_validator.py      # JSON 结构与业务 ID 引用校验
  ai_provider.py              # Provider 抽象接口与输出 Schema
  fake_ai_provider.py         # 无网络测试与演示 Provider
  ai_report_service.py        # AI 报告生成链路编排
  ai_prompt.py                # 冻结的任务与输出约束
  providers/openai_provider.py # OpenAI Provider 与配置读取
data/sample_sales_data.csv    # 示例经营数据
scripts/generate_sample_data.py
tests/                        # pytest 与 Streamlit AppTest
docs/PROJECT_CONTEXT.md       # 项目状态恢复文档
docs/AI_EVALUATION_RESULT.md  # 黄金场景评估记录与 Demo 准备
...
- 当前版本已实现 OpenAI Provider 和 AI Report 页面，但尚未完成真实 OpenAI 报告质量评估，自动测试也不调用真实 API。
- AI 不直接读取原始 CSV，只接收白名单 KPI、结构化诊断结果和受控候选目录。
- 指标计算和问题判断来自 Pandas 与确定性规则系统。
- AI 只能从当前 Finding 允许的原因和行动中选择，并以“待验证假设”表达原因；它不能修改 Finding Priority。
- Validator 第一版只校验 JSON 结构和业务 ID 引用，不审核自然语言事实、因果关系或内容安全。
...
AI Report MVP 已完成。下一步不新增产品功能，优先完成真实 OpenAI 黄金场景评估、典型报告与页面截图等 Demo 素材，以及用于面试讲解产品价值、混合架构和能力边界的文档。
